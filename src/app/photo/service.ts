...
export async function getPhotoList(): Promise<PhotoItemType[]> {
    return photos;
}

export async function getPhotoItem(pid: string): Promise<PhotoItemType> {
    return photos.find(({ id }) => id === pid) || photos[0];
}