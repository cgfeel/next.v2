...
**解决办法：** 
 ...
 - 服务端渲染，用原生`<a>`标签代替`<Link>`组件（推荐），缺点：浏览器会有明显的刷新感
 - 客户端渲染，异步给链接添加`hash`值，缺点：导航链接会多出一串随机的`hash`值
 - `server action`，缺点：只接受`post`请求
...
- 这就意味着在使用并行路由的时候，涉及到实时状态同步的部分，推荐在本地进行异步`fetch`请求，而不是在服务端进行；
...
我对比了服务器渲染的结果，并没有什么不同（画圈的部分是原生HTML）。为什么交互行为不一样，实在想不通了，如果有谁知道麻烦通过issue告知我一声：

![image](https://github.com/cgfeel/next.v2/assets/578141/e7edcc2b-574e-4870-8e9c-2244de99926d)
